...
import { AnalyticsRow } from "../lib/api";
...
export function EfficiencyQuadrants({ rows, yieldSplit, nSplit }: { rows: AnalyticsRow[]; yieldSplit: number; nSplit: number }) {
  ...
  return (
    <div className="glass-card-master p-4 h-[280px] flex flex-col">
      ...
      <div className="grid grid-cols-2 gap-2 flex-1 min-h-0">
        {QUADRANTS.map((q) => (
          <div key={q.key} className="rounded-xl px-2.5 py-2 flex flex-col justify-between min-h-0" style={{ background: q.card, border: "1px solid rgba(12,32,18,0.05)" }}>
